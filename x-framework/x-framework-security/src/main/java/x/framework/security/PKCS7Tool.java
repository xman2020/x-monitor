...
/**
 * PKCS7Tool.java pkcs7格式签名工具 感谢不知名的贡献者，本代码未做任何改动
 * 
 * @version 1.0
 * @author SUNY update for IBM JDK
 */
public class PKCS7Tool {
        ...
        if (keyStore.isKeyEntry(keyAlias)) {
            ...
            for (int i = 0; i < certs.length; i++) {
                if (!(certs[i] instanceof X509Certificate))
                    throw new GeneralSecurityException("Certificate[" + i + "] in chain '" + keyAlias
    ...
    private byte[] specialTrim(byte[] data) {
        int trueSize = 0;
        for (byte b : data) {
            if (b != 13 && b != 10) {
                trueSize++;
        ...
        byte[] trueData = new byte[trueSize];
        int count = 0;
        for (byte b : data) {
            if (b != 13 && b != 10) {
                trueData[count] = b;
    ...
    public String sign(byte[] data) throws Exception {
        ...
        byte[] trueData = this.specialTrim(data);
        Signature signer = Signature.getInstance(signingAlgorithm);
        signer.initSign(privateKey);
        signer.update(trueData, 0, trueData.length);
        ...
        Constructor contentInfoConstructor = ContentInfo.class.getConstructor(new Class[] { data_oid.getClass(),
                derValue });
        ...
        // PKCS7
        Constructor pkcs7Constructor = PKCS7.class.getConstructor(new Class[] { digestAlgorithmIds.getClass(),
                ContentInfo.class, X509Certificate[].class, signerInfos.getClass() });
        PKCS7 p7 = (PKCS7) pkcs7Constructor.newInstance(new Object[] { digestAlgorithmIds, contentInfo, certificates,
                signerInfos });
        ...
        // Base64编码
        String signed = (new BASE64Encoder()).encode(baout.toByteArray());
    ...
    public void verify(String signature, byte[] data, String dn) throws IOException, NoSuchAlgorithmException,
        ...
        // 验证签名本身、证书用法、证书扩展
        SignerInfo[] sis = p7.verify(trueData);
    ...
    private static void init() {
        ...
        try {
            if (vendorUC.indexOf("ORACLE") >= 0 || vendorUC.indexOf("SUN") >= 0) {
                jvm = 'S';
